...
```{r , MatrizSpearman}
# Convertir ordinales a numérico
...
# Agregar puntaje global
matriz_spearman_input <- cbind(
  vars_ordinales_num,
  Global = icfes_corr$PUNT_GLOBAL
)

```

```{r}
cor_spearman <- cor(matriz_spearman_input, method = "spearman")
cor_spearman
```

```{r , MatrizKendall}
cor_kendall <- cor(matriz_spearman_input, method = "kendall")
cor_kendall
...
# Renombrar Spearman
rownames(cor_spearman) <- nombres_ordinales_global
colnames(cor_spearman) <- nombres_ordinales_global

# Renombrar Kendall
rownames(cor_kendall) <- nombres_ordinales_global
colnames(cor_kendall) <- nombres_ordinales_global
```
